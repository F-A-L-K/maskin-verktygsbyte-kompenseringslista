            ...
            {sortedCompensations.length === 0 ? (
              ...
              sortedCompensations.map((comp, index) => (
                <TableRow key={comp.id} className={index % 2 === 0 ? "bg-background" : "bg-muted/50"}>
                  ...
                  <TableCell className="text-center">{format(comp.timestamp, "yyyy-MM-dd HH:mm")} </TableCell>
                  <TableCell className="text-center">{comp.coordinateSystem || "-"} | {comp.tool || "-"} | {comp.number || "-"}</TableCell>
